...
from ..core.evaluation import mean_average_precision, mean_class_accuracy, top_k_accuracy
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def load_data_k400(self):
        """Load data from annotation file."""
        save_data_ori = mmcv.load("/home/zhaozhifu/chenzw/gfst/data/k400_hrnet.pkl")
        split_dict, anno_list = save_data_ori['split'], save_data_ori['annotations']#主要改变annotations
        identifier = 'filename' if 'filename' in anno_list[0] else 'frame_dir'
        folder_path = "data/k400/kpfiles/"
        file_list = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.pkl')]
        data_infos = {}
        for file in file_list:
            data = mmcv.load(file)
            for k, v in data.items():
                data_infos[k] = v['keypoint']

        for i in range(len(anno_list)):
            anno_list[i]['keypoint'] = data_infos[anno_list[i]['frame_dir']]
            frame_inds = anno_list[i]["frame_inds"]
            keypoints = anno_list[i]["keypoint"]
            tmp_array = []
            for frame_idx in range(anno_list[i]["total_frames"]):
                frame_keypoints = keypoints[frame_inds == frame_idx + 1]
                if len(frame_keypoints) == 0:
                    frame_keypoints = np.zeros((2, 17, 3), dtype=np.float16)
                elif frame_keypoints.shape[0] == 1:
                    frame_keypoints = np.vstack((frame_keypoints,
                                                 np.zeros((1, frame_keypoints.shape[1], frame_keypoints.shape[2]),
                                                          dtype=np.float16)))

                sorted_indices = np.argsort(-np.average(frame_keypoints[:, :, 2], axis=1))
                frame_keypoints = frame_keypoints[sorted_indices[:2], :, :]
                tmp_array.append(frame_keypoints)
            anno_list[i]['keypoint'] = np.array(tmp_array).transpose(1, 0, 2, 3)
        save_data_ori['annotations'] = anno_list
        with open('k400_preprocessed.pkl', 'wb+') as f:
            data = mmcv.dump(save_data_ori, f, file_format='pkl')
    ...
    def prepare_train_frames(self, idx):
            ...
            if not isinstance(pack, dict):
                ...
                for k in data:
                    try:
                        ...
                        self.cli = Client(self.mc_cfg, serde=serde.pickle_serde)
                        self.cli.set(k, data[k])
            results['keypoint'] = self.data_infos[key]
            for k in pack:
                results[k] = pack[k]
        ...
        #
        # for frame_idx in range(results["total_frames"]):
        #     frame_keypoints = keypoints[frame_inds == frame_idx+1]
        #     if len(frame_keypoints) == 0:
        #         frame_keypoints = np.zeros((2, 17, 3), dtype=np.float16)
        #     elif frame_keypoints.shape[0] == 1:
        ...
        # results["keypoint"] = tmp_array.transpose(1,0,2,3)
    ...
    def prepare_test_frames(self, idx):
        ...
        # tmp_array = np.array(tmp_array)
        # results["keypoint"] = tmp_array.transpose(1, 0, 2, 3)
        return self.pipeline(results) #丢进pipeline
