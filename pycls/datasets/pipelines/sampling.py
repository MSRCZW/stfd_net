...
class UniformSampleFrames:
    ...
    def _get_test_clips(self, num_frames, clip_len):
        ...
        for i in range(self.num_clips):#num_clips = 10
            ...
            if num_frames < clip_len:
                ...
                offset[inds] = 1 #超出
                offset = np.cumsum(offset)
                inds = basic + offset[:-1] #basic 0~100 offset 0~num_frames - clip_len，实现随机抽取
            else:
            ...
            all_inds.append(inds + off)#会存储num_clips个数组，数组内有clip_len个start_ind~num_frames的元素
            num_frames = old_num_frames

        return np.concatenate(all_inds) #组合为1000个start_ind~num_frames的元素
    ...
    def __call__(self, results):
        ...
        if 'keypoint' in results:
            ...
            coeff = np.array([transitional[i] for i in inds_int])#将1000个抽样帧中对应的transitional保存下来
        ...
        return results #results数据包含了原始骨架的帧数以及frameindex
