...
class PoseDecode:
    ...
    def _load_kp(kp, frame_inds):
        return kp[:, frame_inds].astype(np.float32)#产生对应1000个idx的骨骼数据
...
class FormatGCNInput:
    ...
    def __call__(self, results):
        ...
        if keypoint.shape[0] < self.num_person:
            pad_dim = self.num_person - keypoint.shape[0]#与双人需要添加的pad_dim
            pad = np.zeros((pad_dim, ) + keypoint.shape[1:], dtype=keypoint.dtype)#增加多一个人的数据维度
        ...
        results['keypoint'] = np.ascontiguousarray(keypoint)#为每个视频产生了10个增强的100帧骨骼数据
        return results
